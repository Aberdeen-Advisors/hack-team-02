...
Impact / Redesign / Training / Communication / Adoption); `showDeckProvenance`
is still threaded as a prop into `TrainingView`, `CommunicationView`, and
`PlanView` from `project.hasDeckReproduction`, but is never read inside any of
their bodies — dead wiring from a removed feature. There is no "value stream"
field in any data file and no such filter in `PortfolioView`. Verify any claim
in `docs/CONTEXT.md` against the actual code before repeating it.
...
not line numbers, since the file will keep growing:

1. **`SCORING ENGINE`** — `THRESHOLD`, `NOTE`, `IMPACT_FACTORS`, `RISK_FACTORS`,
   `ANCHORS`, `scoreRole`, `assignTier`. Deterministic JS, never a model call.
2. **`TIER PACKAGES + DOSES`** — `DOSES` (every prescribed dose, one object),
   `ASSUMPTIONS` (costing/planning constants), and the Aberdeen delivery
   material: `TRAINING_METHODS`, `JOB_AID_BANDS`, `COMMS_MILESTONES`,
   `SUPPORT_LAYERS`, `ADOPTION_METRICS`, `CADENCE`.
3. **`PORTFOLIO AGGREGATES + RESOURCED PLAN + ROI`** — `aggregate`,
   `roiModel`, `roiTrend`, `resourcedPlan`.
4. **`DATA`** — the embedded dataset seed (regenerated by
   `tools/embed-seed.js`), `loadRolesForProject`, and **`PROJECTS`** — the
   picker's registry of the three example projects plus the blank
   new-project wizard path.
5. Generation client (`suggestScoresForRole`, calls `api/suggest-scores`),
   presentation helpers, then every React component and the five views,
   ending in `App()` and `ReactDOM.createRoot(...).render(<App/>)`.
